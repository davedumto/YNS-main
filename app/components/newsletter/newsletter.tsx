  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          "🎉 Successfully joined the Newsletter! Welcome aboard!",
          {
            duration: 4000,
            style: {
              background: "#10B981",
      ...
      >
        <div className="flex flex-col items-center lg:items-start lg:gap-[2em] gap-[1em]">
          ...
          <p className="font-[400] text-xl text-white lg:max-w-xl text-wrap text-center lg:text-left">
            Want to stay updated on our outreach programs and impact stories?
            Join our newsletter and be the first to hear how Young and Skilled
            Initiative is empowering students across communities. Let’s grow
            together!
          </p>
        </div>
        <form
          onSubmit={handleSubmit}
          className="flex flex-col lg:flex-row gap-[2em] items-center lg:items-start mt-[2em]"
        >
          <input
            type="text"
            value={formData.name}
